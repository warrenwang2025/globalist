...
const ALLOWED_BLOCK_TYPES = [
  "text", "heading", "quote", "list", "image", "video", "embed", "audio"
];
...
function _isValidBlockArray(blocks: any): boolean {
  ...
  for (const block of blocks) {
    if (typeof block !== 'object' || !block.type || !block.content) return false;
    if (typeof block.type !== 'string' || !ALLOWED_BLOCK_TYPES.includes(block.type)) return false;
...
export async function handleContentCreation(payload: { headline: string; metaDescription: string; tone: string }) {
  ...
  try {
    ...
    if (!_isValidBlockArray(blocks)) {
      return { success: false, error: "AI returned invalid block structure.", usage: response.usage };
    }
    return { success: true, result: { contentBlocks: blocks }, usage: response.usage };
  } catch (err) {
